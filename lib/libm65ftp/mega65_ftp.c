...
struct m65dirent {
  ...
  struct tm d_mtime;             /* Last modified time */
...
int fat_readdir(struct m65dirent *d, int extend_dir_flag)
{
  ...
  do {
    ...
    memset(&d->d_mtime, 0, sizeof(struct tm));
    d->d_mtime.tm_sec   = (d->de_raw[0x16] & 0x1f) << 1;
    d->d_mtime.tm_min   = (d->de_raw[0x16] >> 5) | ((d->de_raw[0x17] & 0x07) << 3);
    d->d_mtime.tm_hour  = d->de_raw[0x17] >> 3;
    d->d_mtime.tm_mday  = d->de_raw[0x18] & 0x1f;
    d->d_mtime.tm_mon   = ((d->de_raw[0x18] >> 5) | ((d->de_raw[0x19] & 0x01) << 3)) - 1;
    d->d_mtime.tm_year  = (d->de_raw[0x19] >> 1) + 80;
...
unsigned int find_free_cluster(unsigned int first_cluster)
{
  ...
  /* Pass 1: search from first_cluster forward */
  {
    int i = first_cluster / (512 / 4);
    int o = (first_cluster % (512 / 4)) * 4;
    ...
    for (; i < sectors_per_fat; i++) {
      ...
      for (; o < 512; o += 4) {
        if (!(fat_sector[o] | fat_sector[o + 1] | fat_sector[o + 2] | fat_sector[o + 3])) {
          unsigned int cluster = (unsigned int)i * (512 / 4) + (unsigned int)(o / 4);
          return cluster;
        }
      }
      o = 0;
  ...
  /* Pass 2: wrap — search from cluster 2 to first_cluster - 1 */
  if (first_cluster > 2) {
    unsigned int wrap_end = first_cluster - 1;
    int last_i = (int)(wrap_end / (512 / 4));
    int last_o = (int)((wrap_end % (512 / 4)) * 4);
    ...
    for (int i = 0; i <= last_i; i++) {
      ...
      int max_o = (i == last_i) ? last_o : 511;
      for (int o = 0; o <= max_o; o += 4) {
        unsigned int c = (unsigned int)i * (512 / 4) + (unsigned int)(o / 4);
        if (c < 2) continue;
        if (!(fat_sector[o] | fat_sector[o + 1] | fat_sector[o + 2] | fat_sector[o + 3]))
          return c;
      }
